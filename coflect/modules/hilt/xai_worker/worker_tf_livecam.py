...
from coflect.modules.hilt.common.synth_numpy import sample_chw_by_idx
...
from coflect.modules.hilt.common.wire import dequeue_xai, post_event
from coflect.modules.hilt.xai_worker.livecam_tf import (
...
def main() -> None:
    ...
    if args.dataset != "synthetic":
        ...
        ds_classes = int(getattr(torch_ds, "num_classes", args.num_classes))
        if args.num_classes <= 0 or args.num_classes == 10:
    ...
    while True:
        ...
        if torch_ds is not None:
            ...
            if isinstance(x_item, torch.Tensor):
                ...
                x_chw = np.asarray(x_item, dtype=np.float32)
            # CIFAR torch dataset path uses normalize(mean=0.5,std=0.5).
            x_chw = np.clip(x_chw * 0.5 + 0.5, 0.0, 1.0).astype(np.float32, copy=False)
        else:
        ...
        if x_chw.shape[1] != args.image_size or x_chw.shape[2] != args.image_size:
            x_nhwc_resized = tf.image.resize(
                np.transpose(x_chw, (1, 2, 0))[np.newaxis, ...],
                size=(args.image_size, args.image_size),
                method="bilinear",
            ).numpy()[0]
            x_chw = np.transpose(x_nhwc_resized, (2, 0, 1)).astype(np.float32, copy=False)
