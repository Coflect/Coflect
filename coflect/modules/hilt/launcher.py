"""One-command launcher for the full Coflect HILT stack."""
...
class RunConfig:
    """Runtime configuration for launching HILT processes."""
...
def _build_process_specs(cfg: RunConfig) -> list[ProcessSpec]:
    ...
    specs: list[ProcessSpec] = [
        ProcessSpec(
            name="backend",
            module="coflect.modules.hilt.backend.app",
            args=["--host", cfg.host, "--port", str(cfg.port)],
        ),
        ProcessSpec(
            name="forecast",
            module="coflect.modules.hilt.forecast.worker",
    ...
    if cfg.backend == "torch":
        ...
        specs.append(
            ProcessSpec(
                name="xai_worker",
                module="coflect.modules.hilt.xai_worker.worker_torch_livecam",
                args=xai_args,
            )
        )
        specs.append(
            ProcessSpec(
                name="trainer",
                module="coflect.modules.hilt.trainer.train_torch",
                args=trainer_args,
            )
        )
        return specs
    ...
    specs.append(
        ProcessSpec(
            name="xai_worker",
            module="coflect.modules.hilt.xai_worker.worker_tf_livecam",
            args=xai_args,
        )
    )
    specs.append(
        ProcessSpec(
            name="trainer",
            module="coflect.modules.hilt.trainer.train_tf",
            args=trainer_args,
        )
    )
    return specs
...
def _parse_args() -> RunConfig:
    ...
    ap.add_argument("--log_dir", type=str, default="./.coflect_logs/hilt")
...
def main() -> None:
    """Entry-point for `coflect-hilt-run`."""
    cfg = _parse_args()
    ...
    print(f"[coflect-hilt-run] backend={cfg.backend} ui={ui_url}")
    print(f"[coflect-hilt-run] logs={log_dir}")
