...
app = FastAPI(title="Coflect HILT Event Server")
...
async def ui_index() -> FileResponse:
    """Serve the built-in static HILT dashboard without Node tooling."""
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Coflect HILT backend server")
    parser.add_argument("--host", default="0.0.0.0")
    parser.add_argument("--port", type=int, default=8000)
    args = parser.parse_args()
    uvicorn.run("coflect.modules.hilt.backend.app:app", host=args.host, port=args.port)
