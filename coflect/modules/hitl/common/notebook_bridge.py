"""Notebook-friendly bridge for live HITL UI integration.
...
from coflect.modules.hitl.common.instruction_parser import ParsedInstruction, parse_instruction
from coflect.modules.hitl.common.messages import XaiRequestPayload
from coflect.modules.hitl.common.wire import enqueue_xai, get_feedback, post_event
...
class NotebookBridgeConfig:
    """Configuration for notebook-to-HITL bridge behavior."""
...
class NotebookHITLBridge:
    """Small stateful bridge for streaming metrics and consuming feedback.

    Example:
        >>> bridge = NotebookHITLBridge(NotebookBridgeConfig(backend="torch"))
    ...
    def _build_xai_cmd(self) -> list[str]:
        ...
        if backend == "torch":
            cmd = _prefix("coflect.modules.hitl.xai_worker.worker_torch_livecam") + [
                "--server",
                self.config.server,
                "--dataset",
        ...
        if backend == "tensorflow":
            cmd = _prefix("coflect.modules.hitl.xai_worker.worker_tf_livecam") + [
                "--server",
                self.config.server,
                "--dataset",
                self.config.xai_dataset,
        ...
        if not raw.is_absolute():
            ...
            candidates.extend(
                [
                    cwd / raw,
                    cwd / name,
                    cwd / "examples" / "hitl" / name,
                    cwd / "HILTM" / "examples" / "hitl" / name,
                ]
            )
            try:
            ...
            if repo_root is not None:
                candidates.extend(
                    [
                        repo_root / raw,
                        repo_root / "examples" / "hitl" / name,
                        repo_root / "HILTM" / "examples" / "hitl" / name,
                    ]
                )
