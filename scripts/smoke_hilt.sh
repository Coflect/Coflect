...
coflect-hilt-backend --host 127.0.0.1 --port 8000 >"$BACKEND_LOG" 2>&1 &
BACKEND_PID=$!
sleep 2
...
if [[ "$BACKEND" == "tensorflow" ]]; then
  ...
  TRAINER_CMD=(coflect-hilt-trainer-tf --server "$SERVER" --steps 200 --xai_every 50 --snapshot_every 100)
  FORECAST_CMD=(coflect-hilt-forecast-worker --server "$SERVER" --backend tensorflow)
  WORKER_CMD=(coflect-hilt-xai-worker-tf --server "$SERVER" --xai_method consensus)
elif [[ "$BACKEND" == "torch" ]]; then
  TRAINER_CMD=(coflect-hilt-trainer-torch --server "$SERVER" --steps 200 --xai_every 50 --snapshot_every 100 --dataset "$DATASET")
  FORECAST_CMD=(coflect-hilt-forecast-worker --server "$SERVER" --backend torch)
  WORKER_CMD=(coflect-hilt-xai-worker-torch --server "$SERVER" --xai_method consensus --dataset "$DATASET")
