...
This benchmark intentionally disables heavy HILT paths in the Coflect run:
...
def _parse_args() -> BenchConfig:
    ...
    ap.add_argument(
        "--output",
        type=str,
        default="docs/benchmarks/hilt_overhead_longrun_latest.json",
...
def _trainer_cmd(cfg: BenchConfig) -> list[str]:
    ...
    cmd = [
        sys.executable,
        "-m",
        "coflect.modules.hilt.trainer.train_torch",
        "--server",
        server,
        "--device",
        cfg.device,
