"""Utility script for HILT local demo commands.
...
BACKEND_CMD = "coflect-hilt-backend --host 0.0.0.0 --port 8000"
TRAINER_CMDS = {
    "torch": (
        "coflect-hilt-trainer-torch --server http://localhost:8000 "
        "--steps 1000 --xai_every 100 --forecast_every 20"
    ),
    "tensorflow": (
        "coflect-hilt-trainer-tf --server http://localhost:8000 "
        "--steps 1000 --xai_every 100 --forecast_every 20"
    ),
}
FORECAST_CMDS = {
    "torch": "coflect-hilt-forecast-worker --server http://localhost:8000 --backend torch",
    "tensorflow": "coflect-hilt-forecast-worker --server http://localhost:8000 --backend tensorflow",
}
WORKER_CMDS = {
    "torch": "coflect-hilt-xai-worker-torch --server http://localhost:8000 --xai_method consensus",
    "tensorflow": "coflect-hilt-xai-worker-tf --server http://localhost:8000 --xai_method consensus",
}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="HILT demo command helper")
